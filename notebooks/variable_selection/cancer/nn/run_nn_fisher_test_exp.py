...
def run_svm(X_train, X_test, y_train, y_test, cv, logger=None, verbose=1):
    svc_param_grid = {"C": np.logspace(-2, 1, 10), "kernel": ["rbf", "linear", "poly"], "degree": [2, 3]}
    svc_grid_cv = GridSearchCV(estimator=SVC(probability=True), param_grid=svc_param_grid, verbose=verbose,
...
def run_logistic_regression(X_train, X_test, y_train, y_test, cv, logger=None, feats=None, verbose=0):
...
def run_bnn(seed, X_train, X_test, y_train, y_test, J, cv, logger):
    ...
    mixed_sgmcmc = MixedSGMCMC(seed=seed, n_samples=2000, n_chains=1, disc_lr=1e-1, layer_dims=[350],
                               lr_schedule="cyclical", batch_size=20, num_cycles=10)
    ...
    mixed_sgmcmc = MixedSGMCMC(seed=seed, lr_schedule="cyclical", n_samples=2000, n_chains=1, disc_lr=1e-1, contin_lr=1e-5,
                               batch_size=20, layer_dims=[350], num_cycles=10, **sgmcmc_grid_cv.best_params_)
...
def run_seed(seed, X_df, y_df, save_dir, nfeats, net_intr, net_intr_rev):
    np.random.seed(seed)
    ...
    logger.info("Running fisher test with threshold 0.01...")
    idx_sig = fisher_exact_test(X_train_df, y_train_df, thres=0.01).ravel()
    logger.info(f"Num significant features (thr 0.01) at {idx_sig.size}")
    ...
    result_summary_dict = {"seed": [seed, seed, seed, seed], "classifier":
                    ["LR", "SVM", "BNN", "BNN + LR"], "num_feats": [idx_sig.size, idx_sig.size, idx_sig.size, nfeats],
                                    "cv_score": [], "test_score": []}
    ...
    log_best_params_1, log_cv_score_1, log_test_score_1 = run_logistic_regression(X_train, X_test, y_train, y_test, cv,
                                                                                                                logger)
    ...
    bnn_best_params, bnn_disc_mean, bnn_cv_score, bnn_test_score = run_bnn(seed, X_train, X_test, y_train, y_test, J, cv,
                                                                                                        logger)
    ...
    log_best_params_2, log_cv_score_2, log_test_score_2 = run_logistic_regression(X_train, X_test, y_train, y_test, cv,
                                                                                  logger, feats=sel_feat_idx)
    ...
    np.save(f"{save_dir}/fisher_idx_sig_{seed}.npy", idx_sig)
...
def main():
    ...
    for seed in exp_seeds:
        run_seed(seed, X_df, y_df, dir_path, nfeats, net_intr, net_intr_rev)
