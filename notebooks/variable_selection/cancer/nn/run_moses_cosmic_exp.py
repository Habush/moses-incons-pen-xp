...
def run_moses(seed, X_train, X_test, y_train, y_test, out_label, cv, logger=None):
...
def run_seed(seed, X_df, y_df, exp_path, save_dir, num_feats, out_label):
    ...
    X_train_sel, X_test_sel = X_train.iloc[:,selected_idx], X_test.iloc[:,selected_idx]
    ...
    result_summary_df.to_csv(f"{save_dir}/res_summary_cosmic_genes_moses_s_{seed}.csv", index_label=False)
...
def main():
    ...
    for seed in exp_seeds:
        ...
        try:
            run_seed(seed, X_cosmic, y_df, exp_dir_path, save_dir_path, nfeats, out_col)
