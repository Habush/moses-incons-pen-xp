...
class MixedSGMCMC(ClassifierMixin):
    ...
    def __init__(self, seed=1234, disc_lr=1e-5, contin_lr=1e-5, sigma=1.0, eta=0.1, mu=0.1,
                 alpha=0.99, batch_size=50, n_samples=10_000, n_warmup=1_000, n_chains=3,
                 cycle_len=10, lr_schedule="constant", temp=1.0, beta=0.5, layer_dims=None, output_dim=1):
    ...
    def fit(self, X, y, J=None, activation_fns=None):
        ...
        disc_grad_est_fn = generate_discrete_grad_estimator(self.model_, disc_logprior_fn, data_size, self.temp)
        contin_grad_est_fn = geneate_mixed_contin_grad_estimator(self.model_ , contin_logprior_fn, data_size, self.temp)

        disc_step_size_fn = nn_util.make_step_size_fn(self.disc_lr, self.lr_schedule, 0.95, self.n_samples,
                                              self.n_warmup, self.cycle_len)

        contin_step_size_fn = nn_util.make_step_size_fn(self.contin_lr, self.lr_schedule, 0.95, self.n_samples,
                                                self.n_warmup, self.cycle_len)
        ...
        warmup_states, states = inference_loop_multiple_chains(key_samples, kernel, init_state, self.lr_schedule ,data, self.batch_size,
                                                                                ...
                                                                                self.n_chains, cycle_len=self.cycle_len,
                                                                                beta=self.beta)
    ...
    def score(self, X, y):
        ...
        return roc_auc_score(y, pred_probs)
    ...
    def get_params(self, deep=True):
        return {"seed": self.seed, "disc_lr": self.disc_lr, "contin_lr": self.contin_lr,
                ...
                "n_chains": self.n_chains, "lr_schedule": self.lr_schedule, "cycle_len": self.cycle_len,
