...
def run_svm(X_train, X_test, y_train, y_test, cv, logger=None):
...
def run_logistic_regression(X_train, X_test, y_train, y_test, cv, logger=None, feats=None):
...
def run_bnn(seed, X_train, X_test, y_train, y_test, J, cv, logger):
    ...
    sgmcmc_grid_cv = GASearchCV(estimator=mixed_sgmcmc, cv=cv,
                                 param_grid=param_grid, verbose=True, population_size=10, generations=4).fit(X_train, y_train,
                                                                            callbacks=[delta_callback, timer_callback],
                                                                            J=J, activation_fns=["tanh"])
