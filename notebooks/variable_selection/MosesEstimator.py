...
class MosesEstimator(BaseEstimator):
    ...
    def predict(self, X):
        X_df = pd.DataFrame(X)
        if self.cols is None:
            X_df = assign_cols(X_df, append_y=False)
        else:
        ...
        # return res
        if self.ensemble:
            return np.squeeze(scipy.stats.mode(res, axis=0).mode)
    ...
    def _fit(self, moses_params=None):
        ...
        opt = ["-i", self.train_file_, "-o", self.out_file_, "-u", self.out_label_, "-m", f"{self.num_evals}", "-W", "1",
               ...
               f"{self.complexity_ratio}", "--random-seed", f"{self.random_state}", "-H", self.prob,
               "-f", self.log_file_]

        if self.fs_algo is not None:
            opt += ["--enable-fs", "1", "--fs-target-size", f"{self.fs_size}", "--fs-algo",
                        f"{self.fs_algo}", "--fs-n-draw", f"{self.n_draws}", "--fs-threshold",
                        f"{self.threshold}", "--fs-smd-top-size", f"{self.fs_size}"]
            if self.assoc_mat is not None:
                opt += ["--assoc-mat", self.assoc_mat, "--inconsistency-alpha", f"{self.alpha}"]
...
def run_fs_moses(X_train, X_test, y_train, y_test, seed, feats, ex=None):
    results = {"moses_cv_score": [], "moses_test_score": [], "log_cv_score": [], "log_test_score": []}
    print("v2")
    for fts in (pgbar := tqdm(feats)):
        X_s_train, X_s_test = X_train[:,fts].astype(np.int64), X_test[:,fts].astype(np.int64)
        y_train, y_test = y_train.astype(np.int64), y_test.astype(np.int64)
        # MosesEstimator(fs_algo=None, random_state=seed).fit(X_s_train, y_train)
        cv_score = np.mean(cross_val_score(MosesEstimator(fs_algo=None, random_state=seed), X_s_train, y_train))
        moses_est_r = MosesEstimator(fs_algo=None, random_state=seed).fit(X_s_train, y_train)
        test_score = moses_est_r.score(X_s_test, y_test)
        ...
        if ex is not None:
            ex.log_scalar("moses_cv_score", 100*cv_score)
            ex.log_scalar("moses_test_score", 100*test_score)
            ex.log_scalar("log_cv_score", 100*cv_score_2)
            ex.log_scalar("log_test_score", 100*test_score_2)

        pgbar.set_postfix({"m_tr": f"{cv_score:.2f}", "m_te": f"{test_score:.2f}", "l_tr": f"{cv_score_2:.2f}", "l_te": f"{test_score_2:.2f}"})
        moses_est_r.cleanup()

    return pd.DataFrame(results)

def run_fs_logreg(X_train, X_test, y_train, y_test, seed, feats, ex=None):
    results = {"log_cv_score": [], "log_test_score": []}
    for fts in (pgbar := tqdm(feats)):
        X_s_train, X_s_test = X_train[:,np.array(fts)].astype(np.int64), X_test[:,np.array(fts)].astype(np.int64)
        y_train, y_test = y_train.astype(np.int64), y_test.astype(np.int64)
        ...
        pgbar.set_postfix({"l_tr": f"{cv_score:.2f}", "l_te": f"{test_score:.2f}"})
    return pd.DataFrame(results)
