...
def parse_mcmc_var_results(seeds, dir, num_exps):
    res_df = {}
    for seed in seeds:
        res_df[seed] = []

    for i in range(1, num_exps):
        with open(os.path.join(dir, f"{i}/config.json"), "r") as fp:
            config = json.load(fp)

        df = pd.read_csv(os.path.join(dir, f"{i}/result_file"))
        df["var"] = [config["var"]["value"]]
        with open(os.path.join(dir, f"{i}/feat_idx_file"), "r") as fp:
            feats = fp.readline().strip()
            df["feats"] = [feats]

        res_df[config["seed"]].append(df)

    dfs_res = []
    for seed in seeds:
        dfs_res.append(pd.concat(res_df[seed], axis=0))
