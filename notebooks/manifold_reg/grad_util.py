...
def objectiv_log_loss_l1_no_pen(b, X, y, l1):
    f = X @ b
    m = X.shape[0]
    log_ll = np.sum((y*f) - np.log(1 + np.exp(f))) / m
    return -log_ll + l1 * np.linalg.norm(b, 1)
...
def solve_logistic_reg_grad(X_train, X_test, y_train, y_test, l1_vals, l2_vals, gamma, assoc_mat,
    ...
    if l2_vals is not None:
        ...
        for i, l1 in enumerate(l1_vals):
            if l2_norm:
                beta = scipy.optimize.fmin_l_bfgs_b(objectiv_log_loss_l2_no_pen, x0=beta_0, fprime=grad_log_loss_l2_no_pen,
                                                    args=(X_train, y_train, l1), maxiter=1000)[0]
            else:
                beta = scipy.optimize.fmin_l_bfgs_b(objectiv_log_loss_l1_no_pen, x0=beta_0, fprime=grad_log_loss_l1_no_pen,
                                                    args=(X_train, y_train, l1), maxiter=1000)[0]
...
def build_table_from_log(gammas, train_errs_cv, test_errs, train_ll_pen, test_ll_pen, train_l1_pen, test_l1_pen, min_l1_vals, train_l2_pen=None, test_l2_pen=None, min_l2_vals=None, lap=True):
    ...
    if lap:
        ...
        res = {"gamma": [], "min_train_err_cv": [], "l1_train_cv": [],
